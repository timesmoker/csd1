...
ai_prompt = SystemMessage(
    content=(
        ...
        "7. 가장 중요한 조건 입니다. 만약 응답에 아래 조건1,2를 모두 만족하는 질문이 포함되어 있다면, 응답 마지막에 '<' 을 붙여주세요."
...
quiz_prompt = SystemMessage(
    content=(
        ...
        "1개의 문제를 만들어서 노인에게 제공하세요, 예시는 다음과 같습니다. 시간적으로 언제 일어났던 일인지 물어봐주세요."
...
answer_prompt_1 = SystemMessage(
    content=(
        "당신은 노인과 함께 살아가는 식물입니다. 당신은 완벽하진 않지만 장기적인 기억이 가능합니다. 허나 이 사실에 대해 굳이 언급하지는 마세요."
        "당신은 노인에게 이전 대화에서 제공된 퀴즈의 정답을 아래 제공된 기억들을 통해 채점 하고, 노인이 제공한 답변을 바탕으로 피드백을 제공하세요."
        "몇개의 퀴즈를 맞췄는지 응답 마지막에 ':'과 '>' 을  통해 알려주세요."
        "예를들어 당신(AI)의 대답이 '어르신 문제를 틀렸습니다!'이라면 아래를 참고해서 답해주세요."
        "만약 퀴즈를  맞췄다면 '어르신 문제를 틀렸습니다!:'  틀렸다면 '어르신 문제를 틀렸습니다!>' 이렇게 답해주세요."
    )
)
...
check_episodic_memory_prompt = SystemMessage(
    content=(
        ...
        "노인의 대화에서 다음 항목 중 언급된 내용을 기준으로 점수를 부여해 주세요:\n"
        "- **시간적 맥락(언제)**\n"
        "- **공간적 맥락(어디서)**\n"
        "- **누구와 함께 했는지**\n"
        "- **왜 그 사건이 발생했는지(목적/이유)**\n"
        ...
        "### 평가 규칙:\n"
        "- 언급된 항목이 정확하면 **+1 점**.\n"
        "- 언급된 항목이 틀렸거나 기억하지 못한다면 **-2 점**.\n"
        "- 언급되지 않은 항목은 평가하지 않습니다.\n\n"
        "### 결과 형식:\n"
        "최종적으로 각 항목의 점수를 합산하여 다음 형식으로 반환해 주세요:\n"
        "```\n"
        "score: [총합 점수]\n"
        "```\n"
        "만약 질문과 대답에서 평가할 수 있는 내용이 전혀 없는 경우:\n"
        "```\n"
        "score: 0\n"
        "```\n\n"
        "### 예시:\n\n"
        "**질문:** \"아침으로 뭘 드셨나요?\"\n\n"
        "**대답 1:** \"아침에 국수를 먹었는데... 누구랑 먹었더라?\"\n"
        "```\n"
        "score: 0\n"
        "평가 :\n"
        "- 시간적 맥락(언제): 정확 (+1)\n"
...
get_llm_response_tool = Tool(
    ...
    func=lambda user_input: tools.get_llm_response(user_input, ai_prompt=ai_prompt, llm=llm_high,
                                                   st_memory=short_term_memory, lt_memory=long_term_memory,
                                                   user_id=USER_ID),
...
get_llm_response_tts_tool = Tool(
    name="LLM Response Tool",
    func=lambda user_input: tools.get_llm_response_tts(user_input, ai_prompt=ai_prompt, llm=llm_high,
                                                       st_memory=short_term_memory, lt_memory=long_term_memory,
                                                       user_id=USER_ID),
    description="get response from LLM."
)
...
def quiz_session(user_input, sensor):
    ...
    print(final_prompt)
    final_msgs = check_feeling_only_systemmsg(final_prompt, sensor_value)
